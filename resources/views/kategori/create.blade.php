...
@section('content')
    <div class="rounded-2xl bg-white p-6 shadow-sm">
        <h1 class="text-2xl font-semibold text-slate-800">Tambah Kategori</h1>
        <p class="mt-2 text-sm text-slate-500">Buat kategori baru untuk pengaduan masyarakat.</p>

        <form action="{{ route('admin.kategori.store') }}" method="POST" class="mt-6 space-y-4">
            @csrf

            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Nama Kategori</label>
                <input type="text" name="nama_kategori" value="{{ old('nama_kategori') }}" required class="w-full rounded-lg border border-slate-300 px-3 py-2" />
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3" class="w-full rounded-lg border border-slate-300 px-3 py-2">{{ old('deskripsi') }}</textarea>
            ...
            <div>
                ...
                <input type="text" name="icon" value="{{ old('icon') }}" class="w-full rounded-lg border border-slate-300 px-3 py-2" />
                @error('icon')<p class="mt-1 text-sm text-rose-600">{{ $message }}</p>@enderror
            </div>

            <div class="flex items-center gap-2">
                <input type="hidden" name="is_active" value="0" />
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300" />
